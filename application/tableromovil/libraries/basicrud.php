...
class BasiCrud {
	...
	function getPagination($options = array(),$fieldSearch = array())
	{
		...
		$CI = & get_instance();	
		$data = '';
		$config['base_url'] = base_url()."".$CI->router->class."/search_c/";
		...
		//last page	   	
	   	...
	   	if($links) $data= "Total:&nbsp;".$config['total_rows']."&nbsp;|&nbsp;P&aacute;g. ".$CI->pagination->cur_page."&nbsp;de&nbsp;".$num_pages."&nbsp;|&nbsp;".$links;  
	   	return $data; 
  ...
  function _check_captcha($p_captcha,$p_ip_address)
  {
	 ...
	 * @return float data
	 */
	function getPrecio($precio_compra, $porcent_real, $porcent_aumento, $operacion)
	{
		if($operacion == 'sumar')
	 ...
	 * @return float data
	 */
	function getPorcentaje($porcent_real, $porcent_aumento, $operacion)
	{
		if($operacion == 'sumar')
